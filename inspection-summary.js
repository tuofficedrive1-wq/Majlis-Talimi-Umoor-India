...
const getGrade = (p) => {
    ...
    if (val >= 80) return "Mumtaz";
    if (val >= 60) return "Behtar";
...
export async function renderInspectionSummary(assignedJamiaat, db) {
    ...
    try {
        ...
        setupCompactDropdown(gradeDropdownBtn, gradeDropdownContent, 'js-grade-checkbox', 'Tamam (All)');
        ...
        container.innerHTML = `<p class="text-red-500 font-bold p-5">UI Load karne mein error aaya: ${err.message}</p>`;
    }
}
...
async function fetchAndRenderReport(db, assignedJamiaat) {
    try {
        ...
        // Sirf assigned jamiaat ka data filter karna
        ...
        if (rows.length === 0) {
            ...
            rows.forEach((row, index) => {
                const pVal = row.percent !== null && row.percent !== undefined ? parseFloat(row.percent).toFixed(1) + "%" : "-";
        ...
        alert("Error: Data load karne mein masla hua. Console check karein.");
    }
}
    ...
    setTimeout(async () => {
        try {
            if (typeof html2canvas === 'undefined') {
                alert("Image download tool (html2canvas) load nahi hua. Page refresh karein.");
                loader.classList.add('hidden');
                return;
            }
