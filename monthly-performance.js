 ...
 * Fix: Handling undefined user snap and academic years error
 */
    ...
    container.querySelectorAll('.save-teacher-btn').forEach(btn => {
        btn.onclick = async () => {
            ...
            try {
                ...
                // ERROR FIX: Check if user document exists
                if (!userSnap.exists()) {
...
const loadAllTeachers = async (jamiaat, db, currentUser) => {
    try {
        ...
        // ERROR FIX: Check if snap exists before accessing data
                ...
                <div class="border border-slate-200 rounded-2xl overflow-hidden bg-slate-50 transition-all hover:border-indigo-400">
                    <div class="teacher-toggle flex justify-between items-center p-4 cursor-pointer" data-teacher-id="${t.id}" data-jamia-name="${jamia}">
                        ...
                        <div class="flex items-center gap-3">
                            <button class="edit-t-btn text-indigo-500 hover:text-indigo-700 transition-all"><i class="fas fa-edit"></i></button>
                            <button class="del-t-btn text-red-500 hover:text-red-700 transition-all"><i class="fas fa-trash-alt"></i></button>
                    ...
                    <div class="period-container hidden p-5 bg-white border-t border-slate-100 space-y-4">
                        <div class="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                            ...
                            <button class="save-period-btn w-full mt-3 bg-emerald-600 text-white py-2 rounded-xl text-xs font-bold hover:bg-emerald-700 transition-all" data-teacher-id="${t.id}" data-jamia-name="${jamia}">Add Period</button>
                        </div>
                        <div class="period-list-table overflow-x-auto">
                            <table class="w-full text-[11px] border-collapse">
                                ...
                                <tbody class="text-slate-700">${(t.periods || []).map(p => `
                                    <tr class="border-b">
                                        ...
                                        <td class="p-2 border text-center"><button class="text-red-400 hover:text-red-600 del-p-btn" data-p-id="${p.id}" data-t-id="${t.id}" data-jamia-name="${jamia}"><i class="fas fa-times"></i></button></td>
                                    </tr>`).join('')}
                                </tbody>
                            </table>
                        </div>
