...
const setupStructureEvents = (container, db, currentUser, assignedJamiaat) => {
    ...
    container.querySelectorAll('.save-teacher-btn').forEach(btn => {
        btn.onclick = async () => {
            const jamiaName = btn.closest('[data-jamia]').dataset.jamia;
    ...
    try {
        ...
        jamiaat.forEach(jamia => {
            ...
            listDiv.innerHTML = jamiaData.teachers.map(t => `
                <div class="border border-slate-200 rounded-2xl bg-slate-50 transition-all">
                    <div class="teacher-toggle flex justify-between items-center p-4 cursor-pointer" data-id="${t.id}" data-jamia="${jamia}">
                        ...
                        <div class="flex items-center gap-3">
                            <button class="del-t-btn text-red-500 hover:text-red-700 p-2"><i class="fas fa-trash-alt"></i></button>
